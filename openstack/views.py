...
from openstack.serializers import VIMSerializer, PortSerializer, \
    HypervisorSerializer, NetworkSerializer, ServerSerializer, \
    InterfaceSerializer
from openstack.utils import  print_values
import keystoneclient.apiclient.exceptions
import keystoneclient.v2_0.client as ksclient
import keystoneclient.v3.client as ks3client
...
#         provider_segmentation_id    = models.CharField(max_length=50,default="unknown")
    ...
    def check_status(self, data):
        try:
            ...
            print data.password
            print data.tenant_name
            print data.user_domain_name
            print data.project_name
            #print data.project_domain_name
                
            if ( data.version == "Kilo" or data.version == "Liberty"):
                keystone = ksclient.Client(auth_url=data.auth_url,
                                       username=data.username,
                                       password=data.password,
                                       tenant_name=data.tenant_name)
                
            if ( data.version == "Mitaka"):
                keystone = ks3client.Client(auth_url=data.auth_url,
                       username=data.username,
                       password=data.password,
                       tenant_name=data.tenant_name,
                       user_domain_name=data.user_domain_name,
                       project_name=data.project_name,
                       project_domain_name=data.user_domain_name)
...
#     
#     queryset = openstack.objects.all()
#     serializer_class = vim_Serializer
#     
#     def get_object(self, pk):
#         print "get_object"
#         try:
#             return openstack.objects.get(pk=pk)
#         except openstack.DoesNotExist:
#             raise Http404
# 
#     def get(self, request, pk, format=None):
#         print "detail get"
#         openstack = self.get_object(pk)
#         serializer = vim_Serializer(openstack)
#         return Response(serializer.data)
#     
...
#         "detail put"
#         openstack = self.get_object(pk)
#         serializer = vim_Serializer(openstack, data=request.data)
